...
filename = "7 - 2.csv"  # Название файла
target_date = datetime.datetime(2017, 5, 12, 0, 0)  # Заданная дата
...
with open(filename, encoding="utf-8") as file:
    ...
    for row in file_reader:
        ...
        for index in score_indices:
            score_value = str(row[index]).replace(",", ".")
            if score_value == "-":
                scores.append(0.0)
            else:
        ...
        if total_score >= 0.6 * max_score:
            ...
            if listener not in successful_listeners:
                successful_listeners.append(listener)

    sorted_listeners = sorted(successful_listeners, key=lambda x: x["test_started"])
