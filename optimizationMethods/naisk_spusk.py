...
def gradient(x): # вычисляет градиент функции в точке x
    h = 0.1 ** 6
    result = []
...
def vector_mul_const(c, x): # умножает вектор x на скаляр с
    return [c * x[i] for i in range(len(x))]


def vector_sum(x1, x2): # складывает 2 вектора
    return [x1[i] + x2[i] for i in range(len(x1))]


def norm(x): # вычисляет норму вектора
    return max([math.fabs(elem) for elem in x])


def get_t(x): # определеяет оптимальное значение шага "t" для метода наискорейшего спуска
...
# задаются начальные значения переменных и параметров для алгоритма
N = 1000
xk = [1.5, 1]
...
# итерационный процесс метода наискорейшего спуска
while True:
    gradxk = gradient(xk) # вычисляется градиент в текущей точке xk
    print("\n3.", k, " Вычислим градиент в x", k, "= (", xk[0], ",", xk[1], "):\n\t grad(f(x", k, "))= (", gradxk[0],
          ",", gradxk[1], ")", sep='')
    normgradxk = norm(gradxk) # вычисляется норма градиента
    print("4.", k, " Вычислим ||grad(f(x", k, "))||= ", normgradxk, sep='')

    if normgradxk < eps1: # если норма градиента меньше порогового значения "eps1", то алгоритм останавливается 
        xres = xk
